...
from .pi_client import PiChat, PiStreamFormatter
...
Use tools when they will materially improve correctness or completeness; otherwise answer directly. Prefer pi's built-in shell and file actions for repository work. Prefer dynamic tools such as `pyrun`, `ex`, `sed`, `bash`, and user-exposed `&`name`` callables when you need live Python state, the active IPython namespace, or an explicitly exposed helper.
...
_status_attrs = "model provider completion_model think code_theme log_exact".split()
...
def _tool_refs(prompt, hist, notes=None):
    ...
    for o in hist:
        names |= _tool_names(o["prompt"])
    for n in notes or []:
        names |= _allowed_tools(n)
    return names
...
async def _astream_to_live_markdown(
    ...
    if final_text:
        console.print(
            _markdown_renderable(_display_text(final_text), code_theme, markdown_cls)
        )
    return final_text
    ...
    if o.get("kind") == "code":
        ...
        if _is_note(source):
            return dict(
                ...
                metadata=dict(
                    ipyagent=dict(kind="code", line=o.get("line", 0), source=source)
                ),
            )
        return dict(
            id=_cell_id(),
            cell_type="code",
            source=source,
            metadata=dict(ipyagent=dict(kind="code", line=o.get("line", 0))),
            outputs=[],
...
class AIMagics(Magics):
    ...
    @line_magic("ipyagent")
    def ipyagent_line(self, line: str = ""):
        return self.ext.handle_line(line)

    @cell_magic("ipyagent")
    async def ipyagent_cell(self, line: str = "", cell: str | None = None):
...
class IPyAIExtension:
    def __init__(
        ...
        shell.user_ns.setdefault("pyrun", RunPython(g=shell.user_ns))
    ...
    def code_context(self, start: int, stop: int) -> str:
        ...
        for _, line, pair in entries:
            source, output = pair
            if not source or _is_ipyagent_input(source):
                continue
            if _is_note(source):
    ...
    def dialog_history(self) -> list:
        ...
        for pid, prompt, response, history_line in self.prompt_records():
            ...
            res.append(
                dict(
                    id=pid, prompt=prompt, response=response, history_line=history_line
                )
            )
    ...
    def save_notebook(self, path) -> tuple[int, int]:
        ...
        nb = dict(
            ...
            metadata=dict(ipyagent_version=1),
            nbformat=4,
            nbformat_minor=5,
        )
    ...
    def _patch_lexer(self):
        ...
        def _lex_document(self, document):
            ...
            if text.startswith(".") or text.startswith("%%ipyagent"):
                return _plain.lex_document(document)
    ...
    def handle_line(self, line: str):
        ...
        return print(
            f"Unknown command: {line!r}. Run %ipyagent help for available commands."
        )
    ...
    async def run_prompt(self, prompt: str):
        ...
        if all_missing:
            warnings = (
                _tag(
                    "warnings",
                    f"The following symbols were referenced but aren't defined in the interpreter: {', '.join(all_missing)}",
                )
                + "\n"
            )
