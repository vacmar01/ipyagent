...
This project is small. Nearly all runtime behavior lives in [ipyai/core.py](ipyai/core.py) and [ipyai/codex_client.py](ipyai/codex_client.py), so getting productive mainly means understanding those files and the tests in [tests/test_core.py](tests/test_core.py).
...
- [ipyai/codex_client.py](ipyai/codex_client.py): local Codex app-server client, stdio JSON-RPC transport, ephemeral thread/turn orchestration, dynamic tool dispatch, and tool/command item rendering
...
- passes those schemas to `ipyai.codex_client.AsyncChat(..., tools=...)`, which exposes them to Codex app-server as `dynamicTools`
...
The tool lookup is intentionally live against the active namespace, so changing a function in the IPython session changes the tool used by subsequent prompts. Async callables are awaited inside `ipyai.codex_client` before their results are returned to app-server.
...
1. uses `ipyai.codex_client.AsyncStreamFormatter` to iterate the response stream
