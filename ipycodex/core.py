...
async def astream_to_stdout(stream, formatter_cls: Callable[..., PiStreamFormatter]=PiStreamFormatter, out=None,
...
def _default_config():
    return dict(model=os.environ.get("IPYAI_MODEL", DEFAULT_MODEL), completion_model=DEFAULT_COMPLETION_MODEL, think=DEFAULT_THINK,
        provider=DEFAULT_PROVIDER, code_theme=DEFAULT_CODE_THEME, log_exact=DEFAULT_LOG_EXACT, prompt_mode=DEFAULT_PROMPT_MODE)
...
class IPyAIExtension:
    def __init__(self, shell, model=None, provider=None, completion_model=None, think=None, code_theme=None, log_exact=None,
    ...
    def _show_help(self):
        cmds = [("(no args)", "Show current settings"), ("help", "Show this help"), ("model <name>", "Set model"), ("provider <name>", "Set provider"), ("think <level>",
    ...
    def handle_line(self, line: str):
        ...
        if clean:
            vals = dict(model=lambda: clean, provider=lambda: clean, completion_model=lambda: clean or DEFAULT_COMPLETION_MODEL, code_theme=lambda: clean or DEFAULT_CODE_THEME,
